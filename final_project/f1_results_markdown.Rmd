...
top_5_per_year_cs %>% ggplot(aes(x = year, y = cs, fill = winner,group = winner)) + geom_line(aes(colour=winner),size = 2) +
  labs( x = "Year", y = "Races Won", title = "Race wins progression of Top 5 most successful drivers over their careers") + 
  scale_y_continuous(breaks = seq(0,100, by=20)) + 
  scale_x_continuous(breaks = seq(1978,2020, by=4)) + 
  theme(legend.position = "none",axis.title=element_text(size=14,face="bold")) +     theme(axis.text = element_text(size = 14), plot.title = element_text(size = 20, face = "bold",vjust = 2.5,hjust = 0.7)) + theme_classic() +
...
gif <- animate(plots_year, 200, fps = 10, duration = 10, width = 800, height = 600, renderer = gifski_renderer("gganim.gif"))
